...
if __name__ == '__main__':
    n = 100
    p = 10
    Atr = np.random.randn(n, p)
    btr = np.random.choice([0, 1], size=n)
    indices = np.random.choice(n, 32, replace=False)

    model = Logistic(Atr, btr, Atr, btr, 0)
    U, S = model.get_rand_nys_appx(indices, 5)
