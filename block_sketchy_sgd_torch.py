...
class BlockSketchySGD(Optimizer):
    
    def __init__(self, params, lr=3e-4, block_rank=3, rho=1e-3, h_recomp_interval=100):
        self.lr = lr 
        self.curr_lr = lr
        ...
        defaults = dict(
            ...
            rho=rho,
            h_recomp_interval=h_recomp_interval,
        )
        super(BlockSketchySGD, self).__init__(params, defaults)
        self.hessian_blocks   = [None for p in self.param_groups[0]["params"]]
    ...
    def step(self, loss_tensor):
        # Compute gradients
        # TODO: assert that there's only one parameter group, otherwise we need to change up some of the code
        gs = torch.autograd.grad(
            loss_tensor, self.param_groups[0]["params"], create_graph=True, retain_graph=True
        )

        for p_idx, (g, p) in enumerate(zip(gs, self.param_groups[0]["params"])):
